...
import (
	...
	helper "github.com/raheem/Ecom/helpers"
...
// 	result := database.DB.Table("products").Select("products.product_name,products.description,products.stock,products.price,brands.brand_name,images.image").
...
func ListProducts(c *gin.Context) {
	...
	result := database.DB.Table("products").Select("products.product_name,products.description,products.stock,products.price,brands.brand_name,categories.category_name,images.image").
		...
		Limit(limit).Offset(offset).
		Scan(&products)
	_ = database.DB.Table("products").Count(&count) // Count the total number of products
	...
	// Create a single JSON response containing both pieces of information
	response := gin.H{
		"Total Products Found": count,
		"Products":             products,
	}
...
// ProductDetails returns product details based on a partial product name.
...
func ProductDetails(c *gin.Context) {
	...
	if partialName == "" {
		c.JSON(http.StatusBadRequest, gin.H{
			"error": "Please provide a partial product name",
		})
		return
	}
	...
	// Use SQL LIKE to match products by partial name
	result := database.DB.Table("products").
		Select("products.product_name, products.description, products.stock, products.price, brands.brand_name, categories.category_name, images.image").
		Joins("INNER JOIN brands ON brands.brand_id = products.brand_id").
		Joins("INNER JOIN categories ON categories.category_id = products.category_id").
		Joins("INNER JOIN images ON images.product_id = products.id").
		Where("products.product_name LIKE ?", "%"+partialName+"%").
		Scan(&products)

	_ = database.DB.Table("products").Where("products.product_name LIKE ?", "%"+partialName+"%").Count(&count)
